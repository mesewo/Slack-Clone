...
const DOC_RULES = {
  clerk: [
    {
      ...
      blocks: [
        ...
        {
          start: "**Can I use it without Clerk?**",
          end: "wire in your own auth solution.",
        },
      ],
    },
    {
      ...
      blocks: [
        {
          start: "**Clerk keyless mode popup**",
          end: "claim application or set env variables",
        },
        {
          start: "**Navigation items not showing**",
          end: "org/permission/role",
        },
      ],
    },
    ...
    { file: "Dockerfile.bun", lines: ["ARG NEXT_PUBLIC_CLERK"] },
    {
      ...
      lines: ["NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY", "CLERK_SECRET_KEY"],
    },
    {
      file: "src/app/about/page.tsx",
      jsxSections: ["Authentication by Clerk"],
    },
    {
  ...
  notifications: [
    ...
    {
      file: "AGENTS.md",
      lines: ["── notifications", "# Remove notification center"],
    },
  ],
  ...
  examples: [
    ...
    },
    {
      ...
      blocks: [
        { start: "**Multi-step forms.**", end: "multi-step-product-form.tsx" },
      ],
...
class FeatureCleanup {
  ...
  async cleanup() {
    ...
    const pkgNow = fs.existsSync(pkgPath)
      ...
      : null;
    const clerkGone =
      this.featuresToRemove.includes("clerk") ||
      !pkgNow?.dependencies?.["@clerk/nextjs"];
    if (clerkGone) {
    ...
    if (this.dryRun) {
      ...
      if (fs.existsSync(nextCache)) {
        ...
        console.log(
          "  🧹 Cleared .next cache (stale route types for removed pages)",
        );
      }
      console.log("\n✨ Cleanup complete!\n");
      ...
      console.log(
        "     Untracked files are not covered by git restore — modified env files",
      );
      console.log(
        "     are backed up as *.cleanup-backup; .clerk/ regenerates on next dev run",
      );
      console.log(
        "  4. Delete scripts/cleanup.js and scripts/cleanup-templates/ if no longer needed\n",
      );
    }
  }
  ...
  cleanConditionalDeps() {
    ...
    const walk = (dir) => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (removedFolders.some((folder) => full.startsWith(folder)))
            continue;
    ...
    for (const dep of CONDITIONAL_DEPS) {
      ...
      const stillUsed = survivors.some((file) =>
        importRe.test(fs.readFileSync(file, "utf8")),
      );
      if (stillUsed) continue;
      if (!this.dryRun) {
        if (pkg.dependencies?.[dep]) delete pkg.dependencies[dep];
        if (pkg.devDependencies?.[dep]) delete pkg.devDependencies[dep];
        if (pkg.devDependencies?.[`@types/${dep}`])
          delete pkg.devDependencies[`@types/${dep}`];
      }
      changed = true;
    ...
    for (const rule of rules) {
      ...
      for (const title of rule.envSections || []) {
        ...
        const t = lines.findIndex(
          (line, i) =>
            line.includes(title) && isBar(lines[i - 1]) && isBar(lines[i + 1]),
        );
        if (t === -1) {
          warn(title);
          continue;
      ...
      for (const text of rule.jsxSections || []) {
        ...
        let end = hit;
        while (end < lines.length - 1 && !lines[end].includes("</section>"))
          end++;
        lines.splice(start, end - start + 1);
      }
      ...
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (updated !== content) {
            ...
            this.log(
              `✅ Remapped dead demo links: ${path.relative(ROOT, full)}`,
            );
          }
        }
      }
  ...
  cleanLaunchJson() {
    ...
    content = content.replace(
      /,?\s*"NEXT_PUBLIC_SENTRY_DISABLED":\s*"[^"]*"/,
      "",
    );
    content = content.replace(/,(\s*[}\]])/g, "$1");
    ...
    if (content !== before) {
      ...
      this.log(
        "✅ Cleaned .vscode/launch.json (removed NEXT_PUBLIC_SENTRY_DISABLED)",
      );
    }
  }
  ...
  async cleanThemes() {
    ...
    if (fs.existsSync(themeConfigPath)) {
      ...
      for (const m of configSource.matchAll(
        /\{\s*name:\s*'([^']*)',\s*value:\s*'([^']*)'\s*\}/g,
      )) {
        existingNames.set(m[2], m[1]);
      }
      for (const theme of themes) {
        if (existingNames.has(theme.value))
          theme.name = existingNames.get(theme.value);
      }
    }
...
`,
  ...
  removeThemeSelector() {
    ...
    if (fs.existsSync(headerPath)) {
      ...
      header = header.replace(
        /\s*<div className='hidden sm:block'>\s*<ThemeSelector \/>\s*<\/div>/,
        "",
      );
      if (header !== before) {
  ...
  pruneFontConfig(keptThemeFile) {
    ...
    const blockRe =
      /const (\w+) = (\w+)\(\{[\s\S]*?variable: '(--font-[a-z0-9-]+)'[\s\S]*?\}\);/g;
    ...
    source = source.replace(
      /export const fontVariables = cn\(([\s\S]*?)\);/,
      (_, entries) => {
    ...
    this.log(
      `✅ Pruned font.config.ts (removed ${removed.length} unused font loaders)`,
    );
  }
}
